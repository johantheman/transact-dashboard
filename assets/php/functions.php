...
function mailer2($event,$no,$date){
	global $conn;
	global $day;
	$sql = "SELECT COUNT(`Email`) AS Metric FROM `transact_reports` WHERE `Event Type` LIKE '%"
	       ."$event"."%' AND `Mailing Name` LIKE '%"
...
function mailer($event,$no,$date){
	global $conn;
	global $day;
	$sql = "SELECT COUNT(DISTINCT `Email`) AS Metric FROM `transact_reports` WHERE `Event Type` LIKE '%"
	       ."$event"."%' AND `Mailing Name` LIKE '%"
	       ."$no"."%' AND `Opt In Date` LIKE '"."$date"."'";
	$result = $conn->query($sql);
	if($result->num_rows > 0){
